...
async def process_callback(hgramid: str, query_url: str) -> None:
    await RedisClient.delete_yandex_token(hgramid=hgramid)
    token = extract_token_data(hgramid=hgramid, yandex_url=query_url)
    await check_and_save_token(hgramid=hgramid,token=token)
